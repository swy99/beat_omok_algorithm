...
class Mock5:
  ...
  * And, convert board into numpy.array and torch.tensor (numpy, tensor)
  ...
  async def play(self, input1=None, input2=None, random_first=True, \
    ...
    """
    async def user_input(game):
      while True:
        v = ""
        if not recv == None:
          await send(self.board)
          v = (await recv()).strip()
        else:
        ...
        try:
          ...
          pass #print("Cannot place stone at {}, {}!".format(v[0], v[1]))
        except Exception:
          pass #print("Wrong input!")
        finally: pass
    ...
    while True:
      if print_intermediate_state:
        print(str(self))
      if pif[self.player] == user_input:
        ret = await user_input(self)
      else:
    ...
    await send(self.board)
    usercolor = 1
    if exchanged: usercolor = 2
    if (winner is not None) and winner > 0 and exchanged:
      winner = 3 - winner
    return usercolor, winner, gg, self.history
